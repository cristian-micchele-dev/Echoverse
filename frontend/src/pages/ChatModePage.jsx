...
export default function ChatModePage() {
  ...
  const [subMode, setSubMode] = useState('solo') // 'solo' | 'duo'
    ...
    <div className={`chat-mode ${visible ? 'chat-mode--visible' : ''} ${exiting ? 'chat-mode--exiting' : ''}`}>
      ...
      <header className="chat-mode-header">
        ...
        <div className="chat-mode-hero">
          ...
          <p className="chat-mode-subtitle">
            {subMode === 'solo'
              ? 'Elegí un personaje y conversá. Sin guión, sin filtros.'
              : 'Elegí dos personajes y hacelos interactuar entre ellos.'}
          </p>
        </div>

        <div className="chat-mode-switcher">
          <button
            className={`chat-mode-switcher__btn${subMode === 'solo' ? ' chat-mode-switcher__btn--active' : ''}`}
          ...
          <button
            className={`chat-mode-switcher__btn${subMode === 'duo' ? ' chat-mode-switcher__btn--active' : ''}`}
            onClick={() => setSubMode('duo')}
          >
            <svg width="14" height="14" viewBox="0 0 22 22" fill="none">
      ...
      {subMode === 'duo' && (
        <div className="chat-mode-duo-preview">
          <div className="chat-mode-duo-preview__icon">
            <svg width="40" height="40" viewBox="0 0 22 22" fill="none">
              <circle cx="7.5" cy="7.5" r="3" stroke="currentColor" strokeWidth="1.5"/>
              <circle cx="14.5" cy="7.5" r="3" stroke="currentColor" strokeWidth="1.5"/>
              <path d="M1 18c0-3 2.8-4.5 6.5-4.5S13 15 13 18" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
              <path d="M13 18c0-3 2.8-4.5 6.5-4.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
            </svg>
          </div>
          <h2 className="chat-mode-duo-preview__title">Chat en Alianza</h2>
          <p className="chat-mode-duo-preview__desc">
            Elegís dos personajes y los hacés interactuar entre ellos. Lo que pase, nadie lo predice.
          </p>
          <ul className="chat-mode-duo-preview__bullets">
            <li>Seleccionás dos personajes de universos distintos</li>
            <li>Cada uno responde con su propia personalidad</li>
            <li>Las respuestas se alternan — el caos es parte del juego</li>
          </ul>
          <button className="chat-mode-duo-preview__cta" onClick={goToDuo}>
            Elegir personajes
